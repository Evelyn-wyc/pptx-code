...
- Vertically center the body composition on ordinary content slides. Define the usable body region as the area below the title or conclusion subtitle and above the footer or source line; exclude those fixed elements from the calculation. Treat the chart, table, diagram, cards, body copy, and their labels as one combined bounding box, then position that group so the remaining space above and below it is approximately balanced. Center the composition as a whole rather than centering each object independently. After rendering, flag slides whose main content is visibly crowded toward the top while a large unused area remains below, and move or enlarge the group within the usable body region without weakening intentional alignment inside the visual.
